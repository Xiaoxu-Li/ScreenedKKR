...
pot_radial_ref = 20;    % constant potential within MT
end


% identical soft Coulomb potentials
if (1)
pot_radial = @(x) -2 ./ sqrt(1+x.^2) .* (x<=R);
pot = @(x) -2 ./ sqrt(1+(x-0.5).^2 ) .* (abs(x-0.5)<=R);   
...
figure
semilogy(1:30, err(1:30), '-o', 'color', [0.3010 0.7450 0.9330], ...
